var ozController = {
  ...
  init: function(){
    ...
    this.map.on('moveend', $.proxy(function(){
      ...
      this.map.eachLayer($.proxy(function(layer){

        if(layer.hasOwnProperty('options')){

          // checking against a polygonisque option. hacky, but works! :D
          if(layer.options.hasOwnProperty('smoothFactor')){

            if(!mapBounds.contains(layer.getBounds()) && !mapBounds.intersects(layer.getBounds())){
  ...
  addOzLayer: function(){
    ...
    this.oz_layer.clearAllEventListeners();
